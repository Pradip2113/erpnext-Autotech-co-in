...
class BOM(WebsiteGenerator):
	...
	def issemifg(self):
		...
		doc=frappe.get_list('Item',fields=["name","is_semi_fg"])
		self.error=0
		for i in self.get('items'):
			temp=''
			for j in doc:
				if j.name==i.item_code and j.is_semi_fg==1:
					doc3=frappe.get_list('BOM',fields=["name","item"],filters={"item":str(j.name)})
					for k in doc3:
						temp=str(k.item)
					if str(j.name)!=temp:
						self.error=2
