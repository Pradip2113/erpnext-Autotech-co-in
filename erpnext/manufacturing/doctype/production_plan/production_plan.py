...
class ProductionPlan(Document):
	...
	def set_correct_amount(self):
		for do in self.get('po_items'):
			original_planned_qty = do.planned_qty
			used_qty = 0
			doc_ppi = frappe.get_all("Production Plan Item" ,filters={'item_code':do.item_code ,'sales_order':do.sales_order},fields =['planned_qty'])
			for xo in doc_ppi:
				used_qty +=  xo.planned_qty
			do.planned_qty =original_planned_qty-used_qty
	...
	def setPlanQty(self):	
		if self.ignore_existing_ordered_qty:
			items_to_remove = []
			for i in self.get('mr_items'):
				if flt(i.actual_qty) < flt(i.required_bom_qty):
					i.quantity = flt(i.required_bom_qty) - flt(i.actual_qty)
					# frappe.msgprint(str(i.quantity))
					frappe.db.set_value(i.doctype, i.name, 'quantity', i.quantity)
				else:
					# if flt(i.quantity) == 0:
					items_to_remove.append(i)

			for item in items_to_remove:
				self.get('mr_items').remove(item)
			for i, item in enumerate(self.get('mr_items'), start=1):
				item.idx = i
	...
	# 				frappe.msgprint(str(temp))
					# self.get('mr_items').remove(self.get('mr_items')[])
					# self.save()
					
					# doc4 = frappe.db.get_list("Material Request Plan Item", fields=["name"],filters={"item_code":str(temp)})
					

					# item_code = temp  # Replace with the item code you want to delete
					# doctype = "Material Request Plan Item"  # Replace with the appropriate doctype of the document

					# # Find the document using the item_code
					# document_name = frappe.db.get_value(doctype, filters={"item_code": item_code})
					...
					# 	# frappe.db.commit()
					# else:
					# 	frappe.msgprint(str(document_name))
