...
frappe.ui.form.on('Production Plan', {
    get_material_for_purchase: function(frm) {
        if (frm.doc.ignore_existing_ordered_qty) {
            frm.doc.mr_items.forEach(function(row) {
                if (flt(row.actual_qty) < flt(row.required_bom_qty)) {
                    var temp = flt(row.required_bom_qty) - flt(row.actual_qty);
                    row.quantity = temp;
                    // frappe.msgprint(row.quantity.toString());
                }
            });
        }
    }
