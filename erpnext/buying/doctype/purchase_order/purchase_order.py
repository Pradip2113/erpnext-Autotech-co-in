...
class PurchaseOrder(BuyingController):
	...
	@frappe.whitelist()
	def list_all_Rates(self,item):
		data = frappe.db.sql("""select tabItemPrice.item_code,ifnull(tabItemPrice.supplier,'') supplier,tabItemPrice.valid_from,tabItemPrice.valid_upto ,tabItemPrice.price_list_rate,
	tabSupplier.supplier_name,tabItem.description
						from `tabItem Price`  tabItemPrice
						inner join `tabSupplier` tabSupplier on
						tabSupplier.name = tabItemPrice.supplier
						inner join `tabItem` tabItem on
						tabItem.name = tabItemPrice.item_code
						where tabItemPrice.price_list ='Standard Buying' 
					and tabItemPrice.item_code = %(item)s; """,{'item': item
						}, as_dict=1)
		if data:
			for row in data:
	...
	def set_filters_IOM(self):
		result_list =[]
		disk =frappe.get_all('Item Supplier', filters={'supplier':self.supplier} ,fields=['parent',])
		if disk:
			for dk in disk:
				result_list.append(dk.parent)
		return result_list
