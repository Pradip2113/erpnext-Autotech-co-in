		...
		if self.stock_entry_type=="Material Transfer for Virtual":
			...
			for row in parent_doc.get("required_items"):
				if(int(row.qsed_quantity)!=int(row.required_qty)):
					...
					if(quantity!=0):
						stock_uom=""
						item_stock_uom=frappe.db.sql("""select stock_uom from `tabItem` where name='{0}'""".format(str(row.item_code)),as_dict=True)
						for i in item_stock_uom:
							stock_uom=i.stock_uom
						
						if(str(row.uom)!=str(stock_uom)):
							child_value = frappe.db.get_value('UOM Conversion Detail', {'parent': row.item_code, "uom":stock_uom}, ['uom', 'conversion_factor'],as_dict=True)
							
							if(child_value):
								conv_factor = child_value.conversion_factor
						else:
								conv_factor=1
						self.append(
								"items",
								{
									...
									"transfer_qty":quantity*conv_factor
